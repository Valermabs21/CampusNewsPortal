...
$stmt = $con->prepare("SELECT cet_comments.comment, cet_comments.date, users.username 
                        FROM cet_comments 
                        JOIN users ON cet_comments.user_id = users.userid 
                        WHERE cet_comments.post_id = ? 
                        ORDER BY cet_comments.date ASC");
...
$stmt_count = $con->prepare("SELECT COUNT(*) as comment_count FROM cet_comments WHERE post_id = ?");
...
<div id="post">
    <div>
        ...
        <a href="cet_like.php?type=post&id=<?php echo htmlspecialchars($ROW['postid']); ?>" style="text-decoration: none;">
       ...
       <a href="javascript:void(0);" onclick="shareOnFacebook('<?php echo htmlspecialchars('http://localhost/campusnewsportal/cet_post.php?id=' . $ROW['postid']); ?>')" style="text-decoration: none;">
        ...
        <div class="comments-section">
            ...
            <?php if (!empty($comments)): ?>
                <div class="comments-list">
                    <?php foreach ($comments as $comment): ?>
                        <div class="comment">
                            <div style="font-weight: bold; color: #405d9b;"><?php echo htmlspecialchars($comment['username']); ?></div><br>
                            <div><?php echo nl2br(htmlspecialchars($comment['comment'])); ?></div><br>
            ...
            <form method="POST" action="cet_comment.php">
